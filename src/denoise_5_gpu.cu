...
// OTTIMIZZAZIONE 2: Blocco 32x8 (sempre 256 thread, ma allineati ai Warp!)
// Questo garantisce che un Warp (32 thread) legga sempre una singola riga contigua.
#define TILE_W 32
#define TILE_H 8             
...
// ---------------------------------------------------------
// KERNEL GPU ULTRA-OTTIMIZZATO (Fix Cache L1 e Coalescing)
// ---------------------------------------------------------
__global__ void adaptive_median_ultra_optimized_kernel(
    ...
    int channels) 
{
    // Tavolo di lavoro dimensionato per il blocco 32x8
    __shared__ uint8_t s_in[SHARED_H][SHARED_W];
    ...
    for (int c = 0; c < channels; c++) {
        ...
        // ========================================================
        // Essendo blockDim.x = 32, questa lettura ora è coalescente al 100%
        ...
        if (gx < width && gy < height) {
            ...
            else 
            {
                ...
                // OTTIMIZZAZIONE 1: Salviamo la Cache L1!
                // Usiamo uint8_t (1 byte) al posto di int (4 byte). 
                // Il valore massimo è 225, quindi uint8_t è perfetto e salva 3/4 di memoria!
...
// ... [LA PARTE HOST DEL C++ RIMANE IDENTICA MA AGGIORNATA PER LA GRIGLIA 32x8] ...

// Puoi copiare e incollare queste due funzioni al posto di quelle vecchie
// o semplicemente aggiornare la dim3 numBlocks nel tuo main.
...
int main(int argc, char *argv[]) {
    ...
    // Griglia configurata per il nuovo blocco 32x8
    dim3 threadsPerBlock(TILE_W, TILE_H);
    dim3 numBlocks((width + TILE_W - 1) / TILE_W,
                   (height + TILE_H - 1) / TILE_H);

    printf("Avvio Filtro GPU (Occupancy 100%% + Coalescing + L1 Fix)...\n");
