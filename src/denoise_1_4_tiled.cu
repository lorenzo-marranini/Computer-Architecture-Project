    ...
    const int* __restrict__ slow_list, const int* __restrict__ slow_count, int area)
{
    ...
    // s_tile:    192 × 32 = 6144 B (cooperative input tile) — IN SHARED MEM
    // s_bbox:    4 ints (min_x, min_y, max_x, max_y) for reduction
    // s_use_tile: 1 byte flag — set if bbox fits in SLOW_TILE_MAX_*
    //
    // NOTE: the histogram is NOT in shared memory.  A correct per-thread
    // histogram would need SLOW_BLOCK_SIZE × 256 B (e.g. 128 KB) which does
    // not fit.  The previous per-warp shared histogram was a BUG (32 threads
    // collided in one slice).  The histogram now lives in per-thread local
    // memory (declared below, inside the active block).  The cooperative
    // TILE stays in shared memory — that is the part that legitimately
    // benefits from the thrust sort's spatial clustering.
    ...
    for (int c = 0; c < channels; c++) {
        ...
        if (active) {
            ...
            // ── PER-THREAD LOCAL HISTOGRAM (the bugfix) ─────────────────────
            // Each thread owns its own 256-byte histogram in local memory.
            // No collision, no shared-memory pressure.
                ...
                if (window_size > 3) {
                    const int r = window_size / 2;
                    {
                        ...
                        for (int wx = -r; wx <= r; wx++) {
                            int gc = max(0, min(gx + wx, width - 1));
                            local_hist[ LOAD_PIXEL(gr_top, gc) ]++;
                            local_hist[ LOAD_PIXEL(gr_bot, gc) ]++;
                        }
                    }
                    {
                        ...
                        for (int wy = -r + 1; wy <= r - 1; wy++) {
                            int gr = max(0, min(gy + wy, height - 1));
                            local_hist[ LOAD_PIXEL(gr, gc_lft) ]++;
                            local_hist[ LOAD_PIXEL(gr, gc_rgt) ]++;
                        }
                    }
                }

                // ── SINGLE-PASS UNROLLED SCAN ───────────────────────────────
                // Reverted from the two-pass early-exit scan: the `break`
                // prevented #pragma unroll and caused warp divergence (32
                // threads converge at different bins), which cost more than
                // the iterations it saved.  A flat 256-iteration unrolled
                // scan with no early exit is faster on the GPU.
