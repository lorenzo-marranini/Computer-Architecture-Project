                ...
                for (int c = 0; c < td->channels; c++) {
                    ...
                    while (window_size <= S_MAX) {
                        ...
                        if ((Z_med - Z_min) > 0 && (Z_med - Z_max) < 0) {
                            ...
                            // STATISTICA: Salviamo la dimensione della finestra che ha avuto successo
                            if (window_size == 3) td->count_3x3++;
                            else if (window_size == 5) td->count_5x5++;
                            ...
                            if (window_size > S_MAX) {
                                ...
                                // STATISTICA: Salviamo la finestra massima usata per resa
                                if (S_MAX == 3) td->count_3x3++;
                                else if (S_MAX == 5) td->count_5x5++;
...
int main(int argc, char *argv[]) {
    ...
    for (int i = 0; i < num_threads; i++) {
        ...
        // Inizializza a 0 i contatori prima di far partire il thread
        td[i].count_3x3 = 0;
        td[i].count_5x5 = 0;
        td[i].count_7x7 = 0;
        td[i].count_larger = 0;
    ...
    // Variabili per aggregare i totali
    long long total_3x3 = 0, total_5x5 = 0, total_7x7 = 0, total_larger = 0;

    for (int i = 0; i < num_threads; i++) {
        pthread_join(threads[i], NULL);
        
        // Sommiamo il lavoro di ogni thread
        total_3x3 += td[i].count_3x3;
    ...
    printf("Blocchi 3x3  utilizzati : %lld (%.2f%%)\n", total_3x3, (double)total_3x3 / total_pixels * 100.0);
