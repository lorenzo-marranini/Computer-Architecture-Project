...
void* adaptive_median_worker(void* arg) {
    ...
    // THE SPLIT: Even threads use Integer ALUs, Odd threads use the FPU
    if (td->thread_id % 2 == 0) {
        
        // --- 1. THE ORIGINAL INTEGER ALU LOGIC ---
        uint8_t buckets[3][256];

        #define ADD_PIXEL_SAFE_RGB_INT(wy, wx) \
            do { \
        ...
        #define ADD_PIXEL_CLAMPED_RGB_INT(wy, wx) \
            do { \
                int ny = y + (wy); \
                int nx = x + (wx); \
                    ...
                    while (window_size <= S_MAX && channels_done != 7) { 
                        ...
                        if (is_safe) {
                            if (window_size == 3) {
                                for (int wy = -1; wy <= 1; wy++) {
                                    for (int wx = -1; wx <= 1; wx++) ADD_PIXEL_SAFE_RGB_INT(wy, wx);
                                }
                            } else {
                                for (int wx = -r; wx <= r; wx++) { ADD_PIXEL_SAFE_RGB_INT(-r, wx); ADD_PIXEL_SAFE_RGB_INT(r, wx); }
                                for (int wy = -r + 1; wy <= r - 1; wy++) { ADD_PIXEL_SAFE_RGB_INT(wy, -r); ADD_PIXEL_SAFE_RGB_INT(wy, r); }
                            }
                        } else {
                            if (window_size == 3) {
                                for (int wy = -1; wy <= 1; wy++) {
                                    for (int wx = -1; wx <= 1; wx++) ADD_PIXEL_CLAMPED_RGB_INT(wy, wx);
                                }
                            } else {
                                for (int wx = -r; wx <= r; wx++) { ADD_PIXEL_CLAMPED_RGB_INT(-r, wx); ADD_PIXEL_CLAMPED_RGB_INT(r, wx); }
                                for (int wy = -r + 1; wy <= r - 1; wy++) { ADD_PIXEL_CLAMPED_RGB_INT(wy, -r); ADD_PIXEL_CLAMPED_RGB_INT(wy, r); }
                            }
                        }
                        ...
                        for (int c = 0; c < 3; c++) {
                            ...
                            if ((Z_med - Z_min) > 0 && (Z_med - Z_max) < 0) {
                                result_color[c] = ((Z_xy[c] - Z_min) > 0 && (Z_xy[c] - Z_max) < 0) ? Z_xy[c] : Z_med; 
                                channels_done |= (1 << c); 
        ...
        // --- 2. THE NEW FLOATING-POINT LOGIC (FPU) ---
        float buckets_f[3][256];

        #define ADD_PIXEL_SAFE_RGB_FLOAT(wy, wx) \
            do { \
                int p_idx = ((y + (wy)) * td->width + (x + (wx))) * td->channels; \
                buckets_f[0][td->in_data[p_idx]] += 1.0f; \
                buckets_f[1][td->in_data[p_idx + 1]] += 1.0f; \
                buckets_f[2][td->in_data[p_idx + 2]] += 1.0f; \
            } while(0)

        #define ADD_PIXEL_CLAMPED_RGB_FLOAT(wy, wx) \
            do { \
                int ny = y + (wy); \
                int nx = x + (wx); \
                if (ny < 0) ny = 0; else if (ny >= td->height) ny = td->height - 1; \
        ...
        while (1) {
            ...
            for (int y = start_row; y < end_row; y++) {
                for (int x = 0; x < td->width; x++) {
                    ...
                    // memset safely zeroes IEEE 754 float bits
                    memset(buckets_f, 0, sizeof(buckets_f));
                    ...
                    while (window_size <= S_MAX && channels_done != 7) { 
                        ...
                        if (is_safe) {
                            if (window_size == 3) {
                                for (int wy = -1; wy <= 1; wy++) {
                                    for (int wx = -1; wx <= 1; wx++) ADD_PIXEL_SAFE_RGB_FLOAT(wy, wx);
                                }
                            } else {
                                for (int wx = -r; wx <= r; wx++) { ADD_PIXEL_SAFE_RGB_FLOAT(-r, wx); ADD_PIXEL_SAFE_RGB_FLOAT(r, wx); }
                                for (int wy = -r + 1; wy <= r - 1; wy++) { ADD_PIXEL_SAFE_RGB_FLOAT(wy, -r); ADD_PIXEL_SAFE_RGB_FLOAT(wy, r); }
                            }
                        } else {
                            if (window_size == 3) {
                                for (int wy = -1; wy <= 1; wy++) {
                                    for (int wx = -1; wx <= 1; wx++) ADD_PIXEL_CLAMPED_RGB_FLOAT(wy, wx);
                                }
                            } else {
                                for (int wx = -r; wx <= r; wx++) { ADD_PIXEL_CLAMPED_RGB_FLOAT(-r, wx); ADD_PIXEL_CLAMPED_RGB_FLOAT(r, wx); }
                                for (int wy = -r + 1; wy <= r - 1; wy++) { ADD_PIXEL_CLAMPED_RGB_FLOAT(wy, -r); ADD_PIXEL_CLAMPED_RGB_FLOAT(wy, r); }
                            }
                        }

                        float target_pos_f = (float)(((window_size * window_size) / 2) + 1); 

                        for (int c = 0; c < 3; c++) {
                            if (channels_done & (1 << c)) continue; 

                            int Z_min = -1, Z_max = -1, Z_med = -1;
                            float count_cumulativo_f = 0.0f;
                            
                            for (int i = 0; i < 256; i++) {
                                if (buckets_f[c][i] > 0.0f) {
                                    if (Z_min == -1) Z_min = i;
                                    Z_max = i; 
                                    count_cumulativo_f += buckets_f[c][i];
                                    if (Z_med == -1 && count_cumulativo_f >= target_pos_f) Z_med = i;
                                }
                            }
...
int main(int argc, char *argv[]) {
    ...
    printf("Filtro Mediano Adattivo - ALU/FPU Split Test (%d thread, chunk=%d)...\n", num_threads, chunk_size);
    ...
    for (int i = 0; i < num_threads; i++) {
        ...
        td[i].thread_id = i;  // <--- Assign the unique ID here
