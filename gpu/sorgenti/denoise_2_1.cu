...
// TILE_H overridabile via -DBLOCK_Y per il sweep
#ifdef BLOCK_Y
#define TILE_H BLOCK_Y
#else
#define TILE_H 4
#endif
#define SHARED_W (TILE_W + 2 * R_MAX) // 32 + 14 = 46
// PADDING: Allarghiamo la riga della memoria condivisa a 64 byte (multiplo di 32)
// per azzerare gli accessi "Uncoalesced" residui.
...
// ---------------------------------------------------------
// KERNEL GPU DEFINITIVO (100% Occupancy, Coalescing, L1 & ALU Fix)
// ---------------------------------------------------------
__global__ void adaptive_median_ultra_optimized_kernel(
    ...
    int channels) 
{
    // Tavolo di lavoro dimensionato con il PADDING (64 byte per riga)
    ...
    for (int c = 0; c < channels; c++) {
        ...
        // ========================================================
        // I warp leggono esattamente 32 pixel in riga. Coalescing garantito!
...
int main(int argc, char *argv[]) {
    ...
    printf("Avvio Filtro GPU (Occupancy 100%% + Coalescing + Sorting Network)...\n");
