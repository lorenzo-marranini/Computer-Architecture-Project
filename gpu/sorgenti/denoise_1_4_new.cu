...
// ---------------------------------------------------------------------------
// Helper device: legge in_data[c, y, x] con clamp ai bordi (SoA).
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Helper device per Istogramma su Registri (Previene il Local Memory Spilling)
// ---------------------------------------------------------------------------
__device__ __forceinline__ void inc_bucket(uint32_t* hist, uint8_t color) {
    uint32_t reg_idx = color >> 2;       // color / 4
    uint32_t byte_pos = color & 3;       // color % 4
    // Essendo S_MAX = 15, l'area max è 225. Un byte non andrà mai in overflow (>255)
    hist[reg_idx] += (1 << (byte_pos * 8)); 
}

__device__ __forceinline__ uint8_t read_bucket(const uint32_t* hist, uint8_t color) {
    uint32_t reg_idx = color >> 2;
    uint32_t byte_pos = color & 3;
    return (hist[reg_idx] >> (byte_pos * 8)) & 0xFF;
}
    ...
    int  area)
{
    ...
    for (int c = 0; c < channels; c++) {
        ...
        if ((z_med - z_min) > 0 && (z_med - z_max) < 0) {
            out_data[base] = ((Z_xy - z_min) > 0 && (Z_xy - z_max) < 0) ? Z_xy : z_med;
        } else {
            out_data[base] = Z_xy; // Placeholder
...
// ===========================================================================
// KERNEL 2 — SLOW PASS  (REGISTER SPILLING FIX APPLICATO)
// ===========================================================================
    ...
    int area)
{
    for (int c = 0; c < channels; c++) {
        ...
        // Allocazione Istogramma su 64 Registri Hardware (anziché 256 byte Local Memory)


        // Allocazione Istogramma su 64 Registri Hardware 
        uint32_t hist[64] = {0};

        int window_size = MIN_WINDOW_SIZE;
        uint8_t final_color = Z_xy;

        // ── Riempimento 3x3 Iniziale ──────────────
        for (int wy = -1; wy <= 1; wy++) {
            int gr = max(0, min(gy + wy, height - 1));
            for (int wx = -1; wx <= 1; wx++) {
                int gc = max(0, min(gx + wx, width - 1));
                uint8_t color_val = in_data[c * area + gr * width + gc];
                inc_bucket(hist, color_val); 
            }
        }
        ...
        while (!decided && window_size <= S_MAX) {
            ...
            if (window_size > 3) {
                ...
                for (int wx = -r; wx <= r; wx++) {
                    ...
                    int gc     = max(0, min(gx + wx, width  - 1));
                    inc_bucket(hist, in_data[c * area + gr_top * width + gc]);
                    inc_bucket(hist, in_data[c * area + gr_bot * width + gc]);
                }
                for (int wy = -r + 1; wy <= r - 1; wy++) {
                    int gr     = max(0, min(gy + wy, height - 1));
                    int gc_lft = max(0, min(gx - r,  width  - 1));
                    int gc_rgt = max(0, min(gx + r,  width  - 1));
                    inc_bucket(hist, in_data[c * area + gr * width + gc_lft]);
                    inc_bucket(hist, in_data[c * area + gr * width + gc_rgt]);
                }
            }

            // ── Scansione Unrolled dell'Istogramma ──────────────
            ...
            #pragma unroll 16
            for (int i = 0; i < 256; i++) {
                uint8_t bucket_val = read_bucket(hist, i);
                if (bucket_val > 0) {
...
int main(int argc, char *argv[]) {
    ...
    printf("=== Two-Pass Adaptive Median Filter (v1.3 with Register Fix) ===\n");
