...
// TILE_SIZE overridabile via -DBLOCK_Y per il sweep.
// IMPORTANTE: in questo file il blocco è SQUARE (TILE_SIZE x TILE_SIZE),
// quindi BLOCK_Y diventa il lato del quadrato.  Non è la stessa convenzione
// di "warps per block @ 32-wide" usata negli altri file — qui il sweep
// esplora dimensioni di tile quadrato, non warps/block.
//   Esempio: BLOCK_Y=16 → blocco 16×16 = 256 thread (8 warp).
//            BLOCK_Y=32 → blocco 32×32 = 1024 thread (32 warp).
#ifdef BLOCK_Y
#define TILE_SIZE BLOCK_Y
#else
